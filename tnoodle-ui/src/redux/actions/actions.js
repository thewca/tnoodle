...
import * as WcaApi from '../../api/WcaApi';
import tnoodle from '../../api/TNoodleApi';
import {
  parseActivityCode,
  getRound,
  formatToScrambleCount,
  checkJson
} from '../../utils/WcaCompetitionJson';
...
export function fetchCompetitionJson(competitionId) {
  return wrapPromiseWithDispatch(
    WcaApi.getCompetitionJson(competitionId),
    'FETCH_COMPETITION_JSON'
  );
}
...
export function generateMissingScrambles(rounds) {
  return (dispatch, getState) => {
    ...
    let { roundsWithMissingSets: rounds } = checkJson(
      getState().competitionJson
    );
    ...
    rounds.forEach(round => {
      ...
      let setsToGenerateCount =
        wcaRound.scrambleSetCount - wcaRound.scrambleSets.length;
      let idsOfSetsToGenerate = [];
      for (let i = 0; i < setsToGenerateCount; i++) {
        idsOfSetsToGenerate.push(`${eventId}-r${roundNumber}-set${i + 1}`);
      }
      let { scrambleCount, extraScrambleCount } = formatToScrambleCount(
        wcaRound.format,
        eventId
      );
          ...
          generatedScrambles => {
            ...
            if (eventId === '333mbf') {
              ...
              for (let i = 0; i < scrambleCount; i++) {
                scrambles = scrambles.concat(
                  generatedScrambles
                    .splice(0, puzzlesPer333mbfAttempt)
                    .join('\n')
                );
              }
              for (let i = 0; i < extraScrambleCount; i++) {
                extraScrambles = extraScrambles.concat(
                  generatedScrambles
                    .splice(0, puzzlesPer333mbfAttempt)
                    .join('\n')
                );
              ...
              extraScrambles = generatedScrambles.slice(
                scrambleCount,
                scrambleCount + extraScrambleCount
              );
            }
  ...
  competitionJson.events.forEach(event => {
    event.rounds.forEach(round => {
      ...
      round.scrambleSets.forEach((scrambleSet, index) => {
        ...
        } Round ${roundNumber} Scramble Set ${prettyIndex}`;
        let request = {
          scrambles: scrambles,
...
export function setScramblePassword(scramblePassword) {
  return (dispatch, getState) => {
    if (getState().isGeneratingScrambles || getState().isGeneratingZip) {
      throw new Error(
        'Cannot set scramble password while generating scrambles or zip file'
      );
    }
...
export function setPuzzlesPer333mbfAttempt(puzzlesPer333mbfAttempt) {
  return (dispatch, getState) => {
    if (getState().isGeneratingScrambles || getState().isGeneratingZip) {
      throw new Error(
        'Cannot set puzzles per 333mbf attempt while generating scrambles or zip file'
      );
    }
...
export function maybeRegenerateScramblesZip() {
  return (dispatch, getState) => {
    let { currentScrambleCount, scramblesNeededCount } = checkJson(
      getState().competitionJson
    );
    if (currentScrambleCount >= scramblesNeededCount) {
      ...
      dispatch({ type: 'CLEAR_SCRAMBLE_ZIP' });
      dispatch(
        downloadScrambles({ pdf: false, password: getState().scramblePassword })
      );
    }
  };
}
...
export function downloadScrambles({ pdf, password }) {
  return (dispatch, getState) => {
    ...
    if (pdf) {
      ...
      return scrambler.fetchZip(title, request, password).then(titleBlob =>
        dispatch({
