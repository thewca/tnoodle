...
import './Layout.css';
import logo from '../../images/tnoodle_logo.svg';
  ...
  connect(state => {
    ...
    class extends Component {
      ...
      render() {
        ...
        if (me) {
          title = <h2>Welcome to TNoodle, {me.name}!</h2>;
          logOutButton = (
            <button
              className="btn btn-outline-light pointer"
              onClick={() => WcaApi.logOut()}
            >
              Log out
            </button>
          );
          ...
          contents = (
            <div>
              <div className="d-flex justify-content-around flex-wrap">
                <a
                  className="btn btn-outline-primary btn-home mt-5"
                  href="/scramble-legacy/"
                >
                  Legacy UI
                </a>
                <button
                  className="btn btn-outline-primary btn-home pointer mt-5"
                  onClick={() => WcaApi.logIn()}
                  title={
                    WcaApi.isUsingStaging()
                      ? 'Remove the staging= url parameter if you want to log in with www.worldcubeassociation.org.'
                      : 'Add a staging=true url parameter if you want to log in with staging.worldcubeassociation.org.'
                  }
                >
          ...
          <div className="app">
            <div
              className={classNames('app-header', {
                error: !!errorMessage,
                alarm: analyzedVersion.alarm
              })}
            >
              <img
                src={logo}
                className={classNames('app-logo', { busy })}
                title={errorMessage}
                alt="TNoodle logo"
              />
...
function analyzeVersion(versionInfo) {
  ...
  var runningVersion =
    new URLSearchParams(window.location.search).get('forceVersion') ||
    versionInfo.running_version;
  ...
  if (versionInfo.ignorableError) {
    ...
    console.log(
      'Ignorable error in checkVersion: ' + versionInfo.ignorableError
    );
    return {};
  }
  ...
  var downloadCurrent = (
    <span>
      <a href={versionInfo.current.download} target="_blank">
        {versionInfo.current.name}
      </a>{' '}
      (
      <a href={versionInfo.current.information} target="_blank">
        details
      </a>
      )
    </span>
  ...
  if (isAllowed && isCurrent) {
    ...
    return {
      message: (
        <div>
          You are running {runningVersion}, which is still allowed, but you
          should upgrade to {downloadCurrent};
        </div>
      ),
      alarm: false
    };
    ...
    return {
      message: (
        <div>
          Scrambles generated with this version of TNoodle must not be used in
          competition.
          <br />
          <small>
            You are on version {runningVersion}, you must use {downloadCurrent}.
          </small>
        </div>
      ),
      alarm: true
