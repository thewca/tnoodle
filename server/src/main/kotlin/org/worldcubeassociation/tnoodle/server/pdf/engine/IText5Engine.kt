...
import org.worldcubeassociation.tnoodle.core.model.pdf.Element
import org.worldcubeassociation.tnoodle.core.model.pdf.Paragraph
import org.worldcubeassociation.tnoodle.core.model.pdf.properties.*
...
object IText5Engine : RenderingEngine<Document> {
    ...
    override fun render(content: Document, password: String?): ByteArray {
        ...
        val fontIndex = content.exposeFontNames()
        ...
        for ((n, page) in content.pages.withIndex()) {
            ...
            if (content.showHeaderTimestamp)
                cb.showHeaderLeft(creationDate.toString(), itextPageSize, headerHeight, page.marginLeft.toFloat())

            page.headerLines?.let {
                cb.showHeaderCenter(it.first, it.second, itextPageSize, headerHeight)
            }

            if (content.showPageNumbers)
                cb.showHeaderRight("${n + 1}/${content.pages.size}", itextPageSize, headerHeight, page.marginRight.toFloat())

            page.footerLine?.let {
                cb.showFooterCenter(it, itextPageSize, footerHeight)
            }
    ...
    private val renderingCache = mutableMapOf<String, MutableMap<Pair<Float, Float>, PdfTemplate>>()
    ...
    private fun renderImage(image: SvgImage, cb: PdfContentByte): Image {
        val pdfTemplate = renderingCache.getOrPut(image.toString()) { mutableMapOf() }
            .getOrPut(image.width to image.height) { cb.renderSvgToPdf(image.svg, image.width, image.height) }

        return Image.getInstance(pdfTemplate)
    }

    private fun PdfContentByte.renderSvgToPdf(svg: String, width: Float, height: Float): PdfTemplate {
        val tp = createTemplate(width, height)
        ...
        try {
            ...
            val parsedSvgDocument = factory.createSVGDocument(null, svg.reader())
    ...
    fun renderWithOutline(documents: List<Document>, password: String? = null): ByteArray {
        ...
        for (document in documents) {
            ...
            val pdfReader = PdfReader(render(document, null))
