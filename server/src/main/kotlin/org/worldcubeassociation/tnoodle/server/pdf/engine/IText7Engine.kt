...
object IText7Engine : RenderingEngine<Document> {
    const val CREATOR_STRING = "iText 7" // TODO is there a cleaner alternative?

    private fun loadFont(name: String, pdfDocument: PdfDocument): PdfFont {
        return PdfFontFactory.createFont("fonts/$name.ttf", PdfEncodings.IDENTITY_H, pdfDocument)
    }
    ...
    override fun render(content: Document, password: String?): ByteArray {
        ...
        for ((n, page) in content.pages.withIndex()) {
            ...
            content.watermark?.let {
                val monoFont = loadFont(Font.MONO, pdfDocument)
                addedPage.addWatermark(it, monoFont, n + 1)
            }
            ...
            page.headerLines?.let {
                addedPage.showHeaderCenter(it.first, it.second, itextPageSize, headerHeight)
            }

            if (content.showPageNumbers)
                addedPage.showHeaderRight("${n + 1}/${content.pages.size}", itextPageSize, headerHeight, page.marginRight.toFloat())

            page.footerLine?.let {
                addedPage.showFooterCenter(it, itextPageSize, footerHeight)
            }
    ...
    private fun renderImage(image: SvgImage, pdfDocument: PdfDocument): Image {
        return SvgConverter.convertToImage(image.svg.byteInputStream(), pdfDocument)
            //.setAutoScale(true)
            .setMaxHeight(image.height)
            .setHeight(image.height)
            .setMaxWidth(image.width)
    ...
    fun renderWithOutline(documents: List<Document>, password: String? = null): ByteArray {
        ...
        for (document in documents) {
            ...
            val renderedBytesIn = render(document, null).inputStream()
