...
object JvmUtil {
    private val LOG = LoggerFactory.getLogger(JvmUtil::class.java)
    ...
    fun wrapMain(args: Array<String>, minHeapSizeMegs: Int, name: String? = null): Boolean {
        LOG.trace("Entering ${JvmUtil::class.java}, method wrapMain, args ${args + minHeapSizeMegs.toString()}")
        ...
        if (os.startsWith("Windows")) {
            ...
            if (launcherDir.isDirectory) {
                ...
                if (!newLauncher.exists()) {
                    try {
                        this.copyFile(java, newLauncher)
    ...
    val callerClass: Class<*>?
        get() {
            ...
            for (i in 2 until stElements.size) {
                ...
                if (JvmUtil::class.java.getPackage() != callerClass.getPackage()) {
                    return callerClass
                }
            }
