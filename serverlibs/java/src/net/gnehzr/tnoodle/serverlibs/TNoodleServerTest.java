	...
	public TNoodleServerTest(String host, int port) {
		...
		requestScramble.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				String scramble;
				try {
					scramble = server.getScramble(puzzle, null);
					scrambleArea.setText(scramble);
					scrambleImg = server.getScrambleImage(puzzle, scramble, width, height, colorScheme);
					PuzzleImageInfo sii = server.getScrambleImageInfo(puzzle, null, width, height);
					faceBoundaries = sii.faces;
					if(colorScheme == null) {
						colorScheme = sii.colorScheme;
					}
					imageLabel.repaint();
				} catch (IOException e1) {
					e1.printStackTrace();
					scrambleArea.setText(e1.getMessage());
				}
			}
		});
