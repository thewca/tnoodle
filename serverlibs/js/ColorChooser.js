...
function hsToIJ(hs) {
    ...
    return {i: (y-1)*.5*color_width*-1, j: (x+1)*.5*color_width};
}
function i_jToHS(i, j) {
    var y = -i/(.5*color_width)+1;
    var x = j/(.5*color_width)-1;
...
function getPoint(e) {
    ...
    return { x: e.pageX - currleft, y: e.offsetY = e.pageY - currtop};
}
    ...
    this.preferredHeight = height + 30;
