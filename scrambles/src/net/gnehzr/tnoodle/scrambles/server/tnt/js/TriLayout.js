var TriLayout = new Class({
	...
	initialize: function(q1, q2, bar, config) {
		...
		this.counterClockwise = new Element('img', { src: 'arrow_rotate_anticlockwise.png' });
		...
		this.clockwise = new Element('img', { src: 'arrow_rotate_clockwise.png' });
		this.clockwise.setStyle('position', 'absolute');
		this.clockwise.setStyle('cursor', 'pointer');
		this.clockwise.fade('hide');
		
		this.swap = new Element('img', { src: 'arrow_switch.png' });
		this.swap.setStyle('position', 'absolute');
		this.swap.setStyle('cursor', 'pointer');
		this.swap.fade('hide');
		
		var showRotate = function() {
			this.counterClockwise.position({relativeTo: this.resizeDiv, edge: 'bottomRight', position: 'topLeft'});
			this.clockwise.position({relativeTo: this.resizeDiv, edge: 'bottomLeft', position: 'topRight'});
			this.swap.position({relativeTo: this.resizeDiv, edge: 'top', position: 'bottom'});
		...
		this.masks.each(function(div) {
			...
			var drag = new Drag.Move(div, {droppables: others });
		...
		var toggleMasks = function toggleMasks() {
			if(this.masksVisible && swappingTris) //we don't disable the masks if swapping is enabled
				return;
...
//		});
		...
		var dragger = new Drag(this.resizeDiv, {snap: 0});
	...
	position: function(divs) {
		...
		//there may be a better way of doing this, but at least this is readable and easy
