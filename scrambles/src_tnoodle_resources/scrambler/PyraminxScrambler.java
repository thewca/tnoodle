...
public class PyraminxScrambler extends Scrambler {
	 ...
	 * @return          a string representing the scramble of a random position
	 */
	public String generateScramble(Random r){
		// Generate a random position
		int randomEdgePerm = r.nextInt(N_EDGE_PERM);
		...
		scramble.append(inverseMoveToString[solution[SCRAMBLE_LENGTH-1]]);
		for (int l=SCRAMBLE_LENGTH-2; l>=0; l--){
		...
		for (int tip=0; tip<4; tip++){
			int dir = r.nextInt(3);
			if( dir < 2 ){
				scramble.append(" " + tipToString[tip*2+dir]);
			}
		}
