...
dd.finlz = function()
{
	...
	//if(dd.uloadFunc) dd.uloadFunc(); //this is infinite recursion, right? --jfly
};
...
dd.recalc = function(d_x)
{
	var d_o, d_i = dd.elements.length; while(d_i--)
	{
		...
		{
			dd.getWH(d_o);
			d_o.defw = d_o.w, d_o.defh = d_o.h; //added by Jeremy Fleischman to help w/ scalable
			if(d_o.div.pos_rel)
			{
				dd.getPageXY(d_o.div);
...
function WINSZ(d_x)
{
	if(d_x)
	{
		...
		window.onresize = WINSZ;//new Function('WINSZ();'); why not use function pointers?! --jfly
	}
...
function isDraggable(el) {
	if(!el)
		return false;
	if(el.className) {
		if(el.className.match(/\btitlebar\b/))
			return true;
		if(el.className.match(/\bbutton\b/))
			return false;
	}
	return isDraggable(el.parentNode);
}
function PICK(d_ev)
{
	dd.recalc(true); //this is probably horrible for performance, but the draggable area isn't being computed correctly --jfly
	...
	var d_o, d_t, d_cmp = -1, d_i = dd.elements.length; while(d_i--)
	{
		...
		if(d_o.visible && 
			...
			dd.e.y >= d_o.y &&
			dd.e.y <= d_o.y+d_o.h &&
			d_o.div.style.display != 'none') { // gotta check for visibility --jfly
...
function RESIZE(d_ev)
{
	...
	if(!dd.whratio)
		var d_h = dd.limH(dd.inWndH(dd.ofy+dd.e.y)-dd.obj.y);
	else {
	
		// rewritten by jeremy fleischman
		d_w -= dd.obj.paddingHorz+2;
		d_h -= dd.obj.paddingVert+2;
		var whratio = dd.obj.minw/dd.obj.minh;
		
		d_h = Math.round(d_w/whratio);
		d_h = Math.max(d_h, dd.obj.minh);
		d_w = Math.max(d_w, dd.obj.minw);
		d_w += dd.obj.paddingHorz+2;
		d_h += dd.obj.paddingVert+2;
	}
	dd.reszTo(d_w, d_h);
...
this.createDraggable = function() {
	...
	/* Not sure what this was for, but it was breaking something --jfly
	...
	dd.setDwnHdl(PICK);
	return d_o; //well, this will only work for 1 element -jfly
}
